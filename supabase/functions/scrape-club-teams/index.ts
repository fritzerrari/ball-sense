...
Deno.serve(async (req) => {
  ...
  try {
    const { club_url, club_id, scope = "all" } = await req.json();
    ...
    // STEP 1: scrape club page → teams list
    console.log("[scrape-club-teams] club page:", club_url);
    const clubData = await fcScrape(
      club_url,
      FC_KEY,
      "Extract all teams (Mannschaften) listed on this club page. Each team has a name, age group (Herren, A-Junioren, B-Junioren, etc.), league/Spielklasse, and a link to the team page. Return team_url as full absolute URL.",
      {
        type: "object",
        properties: {
    ...
    console.log(`[scrape-club-teams] found ${teamsList.length} teams`);
    ...
    const filtered = teamsList.filter((t) => {
      ...
      if (scope === "youth") return /junior|jugend/i.test(ag);
      return true;
    });
    ...
    for (const t of filtered) {
      try {
        ...
        // scrape team page (fixtures + standing snapshot)
        const teamData = await fcScrape(
          t.team_url,
          FC_KEY,
          "Extract upcoming and past fixtures (Spielplan & Ergebnisse), the team's current league position with points and goal difference, and any visible roster (Kader: name, shirt number, position). Dates as ISO YYYY-MM-DD. Times as HH:MM. Mark is_home=true if our team is the home team.",
          {
            type: "object",
            properties: {
