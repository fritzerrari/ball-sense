...
export default function Fields() {
  ...
  const openEdit = (field: any) => {
    ...
    setFormWidth(String(field.width_m));
    setFormHeight(String(field.height_m));
    setDialogOpen(true);
  };
    ...
    <AppLayout>
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Verwalte deine Spielfelder und Kalibrierungen.</p>
          </div>
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            title="Noch keine Plätze angelegt"
            description="Füge deinen ersten Platz hinzu und kalibriere ihn."
            action={<Button variant="heroOutline" onClick={openCreate}>Platz hinzufügen</Button>}
          />
        ) : (
              ...
              <div key={field.id} className="glass-card p-5 space-y-3">
                <div className="flex items-start justify-between">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${
                    ...
                    {isCalibrated(field) ? "Kalibriert" : "Nicht kalibriert"}
                  </span>
                </div>
                ...
                <div className="flex gap-2">
                  <Button variant="heroOutline" size="sm" asChild className="flex-1">
                    <Link to={`/fields/${field.id}/calibrate`}>
                      <Crosshair className="h-4 w-4 mr-1" /> Kalibrieren
                    </Link>
                  </Button>
                  <button onClick={() => openEdit(field)} className="p-2 rounded-lg hover:bg-muted transition-colors">
                    <Pencil className="h-4 w-4 text-muted-foreground" />
                  </button>
                  <button onClick={() => setDeleteId(field.id)} className="p-2 rounded-lg hover:bg-destructive/10 transition-colors">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-card border-border">
          <DialogHeader>
            <DialogTitle className="font-display">{editingField ? "Platz bearbeiten" : "Platz hinzufügen"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            ...
            <Button variant="hero" className="w-full" onClick={handleSubmit} disabled={!formName.trim()}>
              {editingField ? "Speichern" : "Hinzufügen"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <ConfirmDialog
        ...
        title="Platz löschen"
        description="Möchtest du diesen Platz wirklich löschen? Kalibrierungsdaten gehen verloren."
