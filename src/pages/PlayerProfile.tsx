  ...
  return (
    <AppLayout>
      <div className="mx-auto max-w-5xl space-y-6">
        <Link to="/players" className="inline-flex items-center gap-1 text-sm text-muted-foreground transition-colors hover:text-foreground">
        ...
        <div className="glass-card relative overflow-hidden p-5 sm:p-6">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center">
            <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-3xl font-bold font-display text-primary">
            ...
            <div className="flex w-full flex-col gap-2 lg:w-auto">
              ...
              {recentStats[0]?.matches?.id && (
                <Button variant="heroOutline" size="sm" asChild>
                  <Link to={`/matches/${recentStats[0].matches.id}`}>Letztes Spiel <ArrowRight className="ml-1 h-4 w-4" /></Link>
                </Button>
              )}
            </div>
          </div>
        ...
        <div className="space-y-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Komprimiertes Leistungsprofil mit direkten Wegen zu Match- und Detailanalysen.</p>
          </div>
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4">
            {seasonStats.map((item) => (
              <div key={item.label} className="glass-card relative space-y-3 overflow-hidden p-4">
                ...
                <div className="relative flex items-center justify-between">
                  ...
                  <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary/10 text-primary"><item.icon className="h-4 w-4" /></div>
                </div>
                <div className="relative">
                  <p className="text-xs text-muted-foreground">{item.label}</p>
                  <p className="text-xl font-bold font-display leading-tight break-words">{item.value}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          <h2 className="mb-3 text-lg font-semibold font-display">Letzte Spiele</h2>
          {recentStats.length > 0 ? (
            <div className="glass-card overflow-x-auto">
              <table className="min-w-[860px] w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    <th className="px-3 py-3 text-left text-xs font-medium text-muted-foreground">Datum</th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-muted-foreground">Gegner</th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-muted-foreground">km</th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-muted-foreground">Top</th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-muted-foreground">Pässe</th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-muted-foreground">Pass%</th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-muted-foreground">Zwk%</th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-muted-foreground">Ballgew.</th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-muted-foreground">Tore</th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-muted-foreground">Ass.</th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-muted-foreground">Rating</th>
                  </tr>
                </thead>
                <tbody>
                  {recentStats.map((st: any) => {
                    ...
                    return (
                      <tr key={st.id} className="border-b border-border/50 transition-colors hover:bg-muted/20">
                        <td className="px-3 py-3">
                          <Link to={`/matches/${st.match_id}`} className="transition-colors hover:text-primary">{st.matches?.date ? new Date(st.matches.date).toLocaleDateString("de-DE") : "—"}</Link>
                        </td>
                        <td className="px-3 py-3 text-muted-foreground">{st.matches?.away_club_name ?? "—"}</td>
                        <td className="px-3 py-3 font-semibold">{st.distance_km?.toFixed(1) ?? "—"}</td>
                        <td className="px-3 py-3">{st.top_speed_kmh?.toFixed(1) ?? "—"}</td>
                        <td className="px-3 py-3">{st.passes_total ?? 0}</td>
                        <td className="px-3 py-3">{st.pass_accuracy ? `${Math.round(st.pass_accuracy)}%` : "—"}</td>
                        <td className="px-3 py-3">{duelRate !== null ? `${duelRate}%` : "—"}</td>
                        <td className="px-3 py-3">{st.ball_recoveries ?? 0}</td>
                        <td className="px-3 py-3 font-semibold">{st.goals ?? 0}</td>
                        <td className="px-3 py-3">{st.assists ?? 0}</td>
                        <td className="px-3 py-3">{st.rating ? st.rating.toFixed(1) : "—"}</td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        <div>
          <h2 className="mb-3 text-lg font-semibold font-display">Durchschnitts-Heatmap</h2>
          {avgHeatmap ? <HeatmapField grid={avgHeatmap} /> : (
            <div className="glass-card p-8 text-center">
              <Activity className="mx-auto mb-2 h-8 w-8 text-muted-foreground/30" />
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="border-border bg-card">
          <DialogHeader><DialogTitle className="font-display">Spieler bearbeiten</DialogTitle></DialogHeader>
          <div className="mt-2 space-y-4">
            <div>
              <label className="mb-1 block text-sm text-muted-foreground">Name</label>
              <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)} className="w-full rounded-lg border border-border bg-muted px-3 py-2.5 text-sm text-foreground" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1 block text-sm text-muted-foreground">Nummer</label>
                <input type="number" value={formNumber} onChange={(e) => setFormNumber(e.target.value)} className="w-full rounded-lg border border-border bg-muted px-3 py-2.5 text-sm text-foreground" />
              </div>
              <div>
                <label className="mb-1 block text-sm text-muted-foreground">Position</label>
                <select value={formPosition} onChange={(e) => setFormPosition(e.target.value)} className="w-full rounded-lg border border-border bg-muted px-3 py-2.5 text-sm text-foreground">
                  <option value="">Keine</option>
                  {POSITIONS.map((p) => <option key={p} value={p}>{p}</option>)}
                </select>
              </div>
            </div>
            <Button variant="hero" className="w-full" onClick={handleEdit} disabled={!formName.trim()}>Speichern</Button>
          </div>
        </DialogContent>
      </Dialog>
    </AppLayout>
