...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    supabase.from("clubs").select("name, city, league").eq("id", clubId).single().then(({ data }) => {
      if (data) { setName(data.name ?? ""); setCity(data.city ?? ""); setLeague(data.league ?? ""); }
    });
  }, [clubId]);
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="glass-card p-6 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="text-sm text-muted-foreground block mb-1">{t("settings.clubName")}</label>
            ...
            <div>
              <label className="text-sm text-muted-foreground block mb-1">{t("settings.city")}</label>
            ...
            <div>
              <label className="text-sm text-muted-foreground block mb-1">{t("settings.league")}</label>
          ...
          <div>
            <label className="text-sm text-muted-foreground block mb-2">{t("settings.logo")}</label>
            <ClubLogoUpload />
          </div>
            ...
            {(["starter", "club", "pro"] as PlanType[]).map(p => {
              ...
              return (
                <div key={p} className={`p-4 rounded-lg border ${isCurrent ? "border-primary/30 bg-primary/5" : "border-border"}`}>
                  ...
                  <div className="text-xs text-muted-foreground mb-2">{t("settings.perMonth")}</div>
                  <div className="text-xs text-muted-foreground">{c.maxMatches ? `${c.maxMatches} ${t("settings.matchesPerMonth")}` : t("settings.unlimited")}</div>
                  {isCurrent ? (
                    <div className="mt-2 text-xs text-primary flex items-center gap-1"><Check className="h-3 w-3" /> {t("settings.current")}</div>
                  ) : (
